...
class udp extends units\test
{
	...
	function testBufferWantToWait()
	{
		$this
			...
			->then
				->exception(function() { $this->testedInstance->bufferWantToWait(); })
			...
			->then
				->object($this->testedInstance->bufferWantToWait())->isTestedInstance
				->function('socket_set_block')->wasCalledWithArguments($resource)->once
	...
	function testBufferDoNotWantToWait()
	{
		$this
			->given(
				$this->newTestedInstance(new net\host, new net\port),
			...
			->then
				->exception(function() { $this->testedInstance->bufferDoNotWantToWait(); })
					->isInstanceOf('estvoyage\net\socket\exception')
					->hasCode($errno)

			->if(
				$this->function->socket_create = $resource = uniqid(),
				$this->function->socket_set_nonblock = true
			)
			->then
				->object($this->testedInstance->bufferDoNotWantToWait())->isTestedInstance
				->function('socket_set_nonblock')->wasCalledWithArguments($resource)->once

			->if(
				$this->function->socket_set_nonblock = false
			)
			->then
				->exception(function() { $this->testedInstance->bufferDoNotWantToWait(); })
					->isInstanceOf('estvoyage\net\socket\exception')
